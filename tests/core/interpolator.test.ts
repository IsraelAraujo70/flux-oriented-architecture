...
describe("Interpolator", () => {
  ...
  it("should return the expression as is if it is not a string", () => {
    ...
    expect(interpolator.resolve({ foo: "bar" } as any, context)).toEqual({
      foo: "bar",
    });
  });
  ...
  it("should resolve nested properties", () => {
    expect(interpolator.resolve("${input.user.name}", context)).toBe(
      "John Doe",
    );
  });
  ...
  it("should handle multiple interpolations in a string", () => {
    expect(
      interpolator.resolve(
        "User: ${input.user.name} (${input.user.email})",
        context,
      ),
  ...
  it("should return empty string for undefined values in string interpolation", () => {
    expect(interpolator.resolve("Value: ${nonExistent}", context)).toBe(
      "Value: ",
    );
  });
